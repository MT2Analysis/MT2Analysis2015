...
int main( int argc, char* argv[] ) {
  ...
  if( argc<1 ) {
    ...
    bool useMC = true;
    bool closureTest = false;  //true or false?? if false metcut. CAPIRE PERCHE'
 ...
 std::string cond_all_mc = "(id>150&&(id>152||ht<450)&&(id>153||ht<575)&&(id>154||ht<1000)&&(id>155||ht<1500))" ;
 std::string cond_qcd_mc = "(id<160) && (id>=150)" ;
...
//(keep them both to keep the same function structure as in mc)
 ...
 std::string cond_all_data = "nJets>1 && (id&1==1) && run<=280385"  ;
 std::string cond_qcd_data = "nJets>1 && (id&1==1) && run<=280385"  ; 
...
TH1D* set_ratio(double ht_min, double ht_max, std::string  condition, double threshold, TTree* tree, std::string qcd_or_all, bool useMC){
    ...
    if (!useMC && qcd_or_all == "qcd") {
        ...
        oss_small_mc <<"( id >= 300" << "&& ht<" << ht_max << " && ht>" << ht_min <<  " && deltaPhiMin<" << threshold<<" && nJets>1)";
        ...
        std::cout<<"MC: histograms to subtract (without lumiscale)"<<std::endl;
        for (int i = 0; i<=h_big->GetXaxis()->GetNbins(); i++){
            std::cout<<"Bin number: "<<i<<" big dPhi: "<<h_big_mc->GetBinContent(i)<<"  small DPhi: "<<h_small_mc->GetBinContent(i)<<std::endl;
        }
    ...
    std::cout<<"Ratio was set"<<std::endl;
...
void do_fit(double ht_min, double ht_max, std::string  cond_all, std::string cond_qcd, TTree* tree, bool useMC){
    ...
    pow_bg->SetParameter(0, 3352.71);  //output for third area from masciovecchio's code (MC)
    pow_bg->SetParameter(1, -1.78213);
    ...
    if (useMC){
        ...
        legend->AddEntry("h_ratio_qcd","Data after subtraction","PL");
        legend->AddEntry("h_ratio_all","Data","PL");
    }
    ...
    //now, after having plotted the graph, extract parameters from the fit performed one bin left and one bin right
    //so as to compare with the original fit and check its consistency
    //extract parameters
    ...
    pow_bg_right->SetParameter(0, fitResult->GetParameter(0));
    pow_bg_right->SetParameter(1, fitResult->GetParameter(1));
    ...
    pow_bg_left->SetParameter(0, fitResult->GetParameter(0));
    pow_bg_left->SetParameter(1, fitResult->GetParameter(1));
